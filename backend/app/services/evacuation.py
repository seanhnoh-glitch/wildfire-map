...
async def _fema_open_shelters(client: httpx.AsyncClient, lat: float, lon: float) -> list[dict]:
    ...
    try:
        ...
        for feat in (r.json().get("features") or []):
            ...
            detail = ", ".join(x for x in (p.get("address"), p.get("city"), p.get("state")) if x)
            if cap:
...
async def _osm_safe_pois(client: httpx.AsyncClient, lat: float, lon: float) -> list[dict]:
    ...
    try:
        ...
        for el in (r.json().get("elements") or []):
            ...
            detail = ", ".join(x for x in (tags.get("addr:city"), tags.get("addr:state")) if x)
...
def _filter_and_rank_destinations(
    ...
    for d in dests:
        ...
        # for this incident (capacity, services), so prefer it unless it's much
        # farther; then assembly points; synthetic/computed points last.
        if d["source"] == "FEMA":
            pref = -0.25
...
async def plan(
    ...
    async with httpx.AsyncClient(headers=_UA) as client:
        tasks = [_fema_open_shelters(client, lat, lon), _osm_safe_pois(client, lat, lon)]
        if token:
    ...
    for r in all_routes:
        ...
        if better:
            best_by_dest[key] = r
    # Rank clear routes first, then by drive time — but treat an OPEN official
    # shelter as if it were a few minutes closer, so a staffed shelter is
    # recommended over a bare town centroid when the times are comparable.
    def _pref_minutes(r):
        dest = r["destination"]
        if dest["source"] == "FEMA":
            return 8.0
